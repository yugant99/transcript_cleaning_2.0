...
import { CartesianGrid, Line, LineChart, XAxis, YAxis, Legend, Tooltip, BarChart, Bar, LabelList, PolarGrid, RadialBarChart, RadialBar } from "recharts"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"

type WordRepeatRecord = {
  patient_id: string
  week_label: string
  session_type: string
  condition: string
  filename: string
  caregiver_repeats: number
  plwd_repeats: number
...
export default function WordRepeatsPage() {
  const [rows, setRows] = useState<WordRepeatRecord[]>([])
  const [examples, setExamples] = useState<WordRepeatExample[]>([])
    ...
    const load = async () => {
      try {
        const [s1, s2] = await Promise.all([
          fetch("http://localhost:8000/api/word-repeats"),
          fetch("http://localhost:8000/api/word-repeats-examples"),
        ])
        if (!s1.ok || !s2.ok) throw new Error("Failed to load word repeats")
        const d1: WordRepeatRecord[] = await s1.json()
        const d2: WordRepeatExample[] = await s2.json()
  ...
  const byWeek = useMemo(() => {
    ...
    filtered.forEach(r => {
      ...
      cur.caregiver += r.caregiver_repeats
      cur.plwd += r.plwd_repeats
      cur.total += r.total_repeats
  ...
  const topWords = useMemo(() => {
    ...
    filtered.forEach(r => {
      Object.entries(r.by_word || {}).forEach(([w, c]) => {
        counts[w] = (counts[w] || 0) + (c || 0)
      })
    })
  ...
  const donutData = useMemo(() => topWords.map(t => ({ word: t.word, value: t.count })), [topWords])

  if (loading) return <div className="p-6">Loading word repeats...</div>

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Word Repeats</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Repeat Trends Over Time</CardTitle>
        ...
        <Card className="flex flex-col">
          <CardHeader className="items-center pb-0">
            <CardTitle>Top Repeated Words</CardTitle>
            <CardDescription>Most frequent immediate repeats</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 pb-0">
            <ChartContainer config={{ value: { label: "Count" } } as ChartConfig} className="mx-auto aspect-square max-h-[260px]">
              <RadialBarChart data={donutData} innerRadius={30} outerRadius={100}>
                ...
                <RadialBar dataKey="value" nameKey="word" />
                <Legend />
              </RadialBarChart>
            </ChartContainer>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Detailed Word Repeats by File</CardTitle>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Caregiver Repeats</TableHead>
                  <TableHead>PLWD Repeats</TableHead>
                  <TableHead>Total Repeats</TableHead>
              ...
              <TableBody>
                {filtered.map((row, idx) => (
                  <TableRow key={idx} className="hover:bg-muted/30">
                    ...
                    <TableCell className="text-blue-700">{row.caregiver_repeats}</TableCell>
                    <TableCell className="text-green-700">{row.plwd_repeats}</TableCell>
                    <TableCell className="font-medium">{row.total_repeats}</TableCell>
                    <TableCell>{row.total_words.toLocaleString()}</TableCell>
                    <TableCell className="text-amber-700">{row.repeat_rate.toFixed(2)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Repeated words in context</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {examples.slice(0, 40).map((ex, i) => (
              <div key={i} className="rounded-md border p-3">
                <div className="text-sm font-semibold mb-1">{ex.word}</div>
                <div className="text-sm">{ex.context}</div>
